            ...
            <div class="row">
                    ...
                    <div class="product-details-img">
                        <img class="zoompro"
                            src="{{ asset(config('filepath.img_product_path') . $product->images->first()->image_path) }}"
                                data-zoom-image="{{ asset(config('filepath.img_product_path')
                                . $product->images->first()->image_path) }}" alt="zoom"/>
                            ...
                            @foreach ($product->images as $image)
                                <a data-image="{{ asset(config('filepath.img_product_path') . $image->image_path) }}"
                                   data-zoom-image="{{ asset(config('filepath.img_product_path') . $image->image_path) }}">
                                    <img height="100" alt=""
                                        src="{{ asset(config('filepath.img_product_path') . $image->image_path) }}">
                                </a>
                            @endforeach
                        </div>
                ...
                <div class="col-lg-6 col-md-12">
                    <div class="product-details-content">
                        ...
                        @if ($product->quantity < config('attribute_product.quantity_min'))
                            <div class="in-stock">
                                <p>{{ trans('client.available') }}</b>
                                    <span>
                                        <i class="fas fa-times-circle"></i> {{ trans('client.out_of_stock') }}
                                    </span>
                                </p>
                            </div>
                        @else
            ...
            <div class="description-review-wrapper">
                <div class="description-review-topbar nav text-center">
                    <a class="active" data-toggle="tab" href="#des-details1">
                        {{ trans('message.product_description') }}
                    </a>
                                        ...
                                        <div class="star-author-all">
                                            <div class="ratting-author f-left">
                                                <div class="rateit" data-rateit-readonly="true"
                                                     data-rateit-value="{{ $rating->rating }}">
                                                </div>
                                            </div>
                ...
                <div class="product-wrapper">
                    <div class="product-img">
                        <a href="{{ route('shopping.show', $productItem->id) }}">
                            <img alt="" src="{{ asset(config('filepath.img_product_path')
                                . $productItem->images->first()->image_path) }}">
                        </a>
