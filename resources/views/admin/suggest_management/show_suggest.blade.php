                    ...
                    <td>
                        @switch ($suggest->status)
                            @case (config('status.pending'))
                                <span class="badge badge-warning suggest-status-badge badge{{ $suggest->id }}">
                                    {{ trans('message.pending') }}
                                </span>
                                @break
                            @case (config('status.approved'))
                                <span class="badge badge-success suggest-status-badge badge{{ $suggest->id }}">
                                    {{ trans('message.approved') }}
                                </span>
                                @break
                            @case (config('status.canceled'))
                                <span class="badge badge-secondary suggest-status-badge badge{{ $suggest->id }}">
                    ...
                    <td>
                        <div class="custom-control-inline">
                            @if ($suggest->status != config('status.canceled'))
                                ...
                                @endif
                                <form class="delete" action="{{ route('suggests.destroy', $suggest->id) }}" method="post">
                                    @csrf
                                    @method('DELETE')
